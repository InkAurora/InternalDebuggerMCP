...
- `read_memory`, `write_memory`, `memory_verify_failed`, `disassemble`, and unreadable-address `create_signature` failures may include extra native diagnostic fields such as `address`, `requested_size`, `requested_max_bytes`, `memory_reason`, `region_base`, `region_size`, `region_state`, `region_protect`, `win32_error_detail`, and `copy_exception_code`.
